 ...
 *  {string} [id=W1wlwhzIlaCHivyqdvlW]
 *  {string} name
 * }
 */
export default class Widget extends Prototype() {
  ...
  render () {
    ...
    return this.loadDependency().then(msc => {
      msc[this.getAttribute('name')](this.mscWrapper, JSON.parse(`{
        "language": "${this.getAttribute('language') || self.Environment.language}",
        "${this.getAttribute('name')}Id": "${this.getAttribute('id') || 'W1wlwhzIlaCHivyqdvlW'}"
      }`))
